...
export const isSwapTransaction = (instructions: ParsedInstruction[]) => {
  const isSwap = instructions.some((instruction) =>
    instruction.programId.equals(new PublicKey(JUPITER_SWAP_PROGRAM_ID)),
  );
...
export const getSwapTransactionDetails = async (
  ...
  const transferInstructions = getFilteredTransferInstructions(
    innerSwapInstructions.instructions as ParsedInstruction[],
  );
